...
const palletWeightSchema = z
  ...
  .refine(
    isValidPalletWeight,
    `Enter a weight above 0 and no more than ${palletWeightMaximumKg} kg.`
  );

export const palletSetSchema = z
  .object({
    pallets: z
      .array(
        z
      ...
      .superRefine((pallets, context) => {
        ...
        for (const [index, pallet] of pallets.entries()) {
          if (seen.has(pallet.sequenceNumber)) {
            context.addIssue({
              code: "custom",
              message: "Pallet sequence numbers must be unique.",
...
export const palletClearSchema = z
  .object({
    updatedAt: z.string().datetime().optional(),
  })
  .strict();
